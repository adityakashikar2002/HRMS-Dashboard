...
//   const allItems = showDraftActions ? drafts : emails;
...
//                 className="block"
//                 onClick={() => !item.isDraft && onMarkAsRead && onMarkAsRead([item.id])}
//               >
...
const EmailList = ({ 
  ...
  const allItems = showDraftActions ? drafts : 
                   showSentActions ? sentEmails : 
                   emails;
          ...
          {allItems.map((item) => (
            ...
            >
              ...
              <td className="px-4 py-2">
                {item.avatar ? (
                  <img 
                    alt={`Profile ${showSentActions ? item.to : item.sender}`} 
                    className="w-10 h-10 rounded-full" 
                    src={item.avatar} 
                  />
                ) : (
                  <div className="w-10 h-10 bg-purple-500 text-white rounded-full flex items-center justify-center">
                    {showSentActions ? 'Me' : item.senderInitials}
                  </div>
                )}
              </td>
              <td className="px-4 py-2">
                <span className={!item.isRead ? 'font-semibold' : ''}>
                  {showSentActions ? `To: ${item.to}` : item.sender}
                </span>
              </td>
              {item.label && (
                <td className="px-4 py-2">
              ...
              <td className="px-4 py-2">
                <Link 
                  to={item.isDraft ? `/inbox/drafts/${item.id}` : 
                      showSentActions ? `/inbox/sent/${item.id}` : `/inbox/emails/${item.id}`}
                  className="block"
                  onClick={() => !item.isDraft && onMarkAsRead && onMarkAsRead([item.id])}
                >
                  <span className={!item.isRead ? 'font-semibold' : ''}>{item.subject}</span>
