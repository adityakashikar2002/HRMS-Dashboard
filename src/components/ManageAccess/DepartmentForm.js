...
import { toast } from 'react-toastify';
import { motion } from 'framer-motion';
import { FiX, FiBriefcase, FiFileText, FiCheck, FiUsers, FiClock, FiDollarSign } from 'react-icons/fi';
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-6 text-white">
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-semibold flex items-center">
            <FiBriefcase className="mr-3" /> Create New Department
          </h3>
          <button
            onClick={onCancel}
            className="p-1 rounded-full hover:bg-blue-700 transition-colors"
      ...
      <form onSubmit={handleSubmit} className="p-6">
        <div className="mb-6">
          <label className="block text-gray-700 font-medium mb-2 flex items-center">
            <RiShieldUserLine className="mr-2 text-blue-500" /> Department Name*
          </label>
          <div className="relative">
            <input
              type="text"
        ...
        <div className="mb-8">
          <label className="block text-gray-700 font-medium mb-3 flex items-center">
            <FiCheck className="mr-2 text-green-500" /> Default Access Permissions
          </label>
          <div className="flex flex-wrap gap-3">
            {[
              { name: 'Dashboard', icon: <RiShieldUserLine className="mr-1" />, color: 'bg-blue-100 text-blue-700' },
              { name: 'Tasks', icon: <FiCheck className="mr-1" />, color: 'bg-green-100 text-green-700' },
              { name: 'Inbox', icon: <FiFileText className="mr-1" />, color: 'bg-purple-100 text-purple-700' },
              { name: 'Calendar', icon: <FiClock className="mr-1" />, color: 'bg-yellow-100 text-yellow-700' },
              { name: 'Projects', icon: <FiBriefcase className="mr-1" />, color: 'bg-pink-100 text-pink-700' }
            ].map((access, i) => (
              <motion.div
                key={access.name}
                whileHover={{ scale: 1.05 }}
                className={`px-4 py-2 ${access.color} rounded-full text-sm font-medium flex items-center shadow-sm`}
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100">
          <motion.button
            ...
            whileHover={{ x: -2 }}
            whileTap={{ scale: 0.98 }}
            className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-all flex items-center"
          >
