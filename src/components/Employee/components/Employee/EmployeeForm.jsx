...
const EmployeeForm = ({
  ...
  const isCEO = formData.position?.toLowerCase().includes('ceo') || formData.position?.toLowerCase().includes('department head');
  ...
  //   const selectedDept = departments.find(d => d.name === formData.department);
  //   if (selectedDept) {
  //     // Combine department positions with general positions
  //     return [...selectedDept.positions, ...positions.filter(p => !p.department)];
  //   }
  //   return positions;
  ...
  const departmentPositions = useMemo(() => {
    ...
    const selectedDept = departments.find(d => d.name === formData.department);
    if (selectedDept) {
      // Combine department positions with general positions, removing duplicates
      const deptPositions = selectedDept.positions.map(p => ({
        title: p.title,
        level: p.level,
      ...
      const generalPositions = positions.filter(p => 
        !p.department || p.department === selectedDept.name
      );
      
      return [...deptPositions, ...generalPositions];
    }
    return positions;
  }, [formData.department, departments, positions]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
              required
              disabled={isCEO || formData.position?.toLowerCase().includes('department head')}
            >
              <option value="">Select Department</option>
              {departments.map(dept => (
                <option key={dept.id} value={dept.name}>{dept.name}</option>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
              required
              disabled={!formData.department && !isCEO && !formData.position?.toLowerCase().includes('department head')}
            >
              <option value="">Select Position</option>
              {departmentPositions.map((pos, idx) => (
                <option key={idx} value={pos.title}>{pos.title} ({pos.level})</option>
