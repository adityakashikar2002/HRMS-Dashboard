...
const TreeNode = ({ node = {}, onEmployeeClick = () => {} }) => {
  ...
  const getPositionColor = (position, type) => {
    if (type === 'department') return 'bg-gradient-to-r from-indigo-600 to-purple-600';
    if (!position) return 'bg-gradient-to-r from-gray-600 to-gray-500';
    ...
    <div className="flex flex-col items-center relative tree-node">
      ...
      >
        ...
        <h3 className="font-bold text-sm">
          {node.type === 'department' ? node.name : node.position}
        </h3>
        <p className="text-xs">
          {node.type === 'department' ? 'Department' : node.name}
        </p>
        {node.phone && <p className="text-[10px] mt-1">{node.phone}</p>}
        {node.email && <p className="text-[10px]">{node.email}</p>}
      </div>
