...
const CalendarDay = ({ currentDate, events, onEventClick, onTimeSlotClick }) => {
  ...
  const allDayEvents = events.filter(event => 
    event.allDay && isSameDay(event.start, currentDate) && event.isHoliday
  );
  
  const timedEvents = events.filter(event => 
    !event.allDay && isSameDay(event.start, currentDate)
  );
        ...
        <div className="flex flex-col">
          ...
          {allDayEvents.length > 0 && (
            <div className="border-b p-2">
              <div className="font-medium text-sm mb-2">All Day</div>
