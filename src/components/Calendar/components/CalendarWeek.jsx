      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="flex border-b">
          <div className="w-16 flex-shrink-0 border-r">
            <div className="h-8 flex items-center justify-end pr-2 text-xs text-gray-500">
              All Day
            </div>
          </div>
          {days.map(day => {
            ...
            // );
            const dayAllDayEvents = events.filter(event => 
              event.allDay && isSameDay(event.start, day) && event.isHoliday
            );
            
            return (
              <div key={`all-day-${day}`} className="flex-1 border-r last:border-r-0">
                {dayAllDayEvents.map(event => (
                  <div key={event.id} className="px-1 py-0.5">
                    <CalendarEvent 
                      event={event} 
                      onClick={() => onEventClick(event)}
                      compact
